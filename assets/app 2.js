const App = (() => {
  ...
  function getProductImages(product) {
    const raw = [product.image, ...(Array.isArray(product.gallery) ? product.gallery : [])].filter(Boolean);
    return [...new Set(raw)];
  }
...
function productCard(product) {
    ...
    const leadImage = images[0] || product.image || '';
    const isPlaceholder = leadImage.includes('assets/placeholders/');

    return `
      <article class="product-card">
        <a class="image-wrap ${isPlaceholder ? 'is-placeholder' : ''}" href="product.html?slug=${encodeURIComponent(product.slug)}">
          <img src="${leadImage}" alt="${product.name}" loading="lazy" />
          ${isPlaceholder ? `<span class="placeholder-badge">Photo coming soon</span>` : ''}
        </a>
        <div class="content">
          <span class="badge">${product.badge}</span>
          <h3 class="product-card-title"><a href="product.html?slug=${encodeURIComponent(product.slug)}">${product.name}</a></h3>
          <div class="card-meta">
  ...
  async function renderProductPage() {
      ...
      <section class="section">
        <div class="container product-layout">
          <div class="product-gallery">
            <div class="panel product-gallery-main">
              <img id="product-main-image" src="${leadImage}" alt="${product.name}" />
            </div>
            ${images.length > 1 ? `
              <div class="product-thumbs" aria-label="Product image gallery">
                ${images.map((image, index) => `
                  <button class="product-thumb ${index === 0 ? 'active' : ''}" type="button" data-product-thumb="${image.replace(/"/g, '&quot;')}" aria-label="View image ${index + 1} of ${product.name}">
    ...
    document.querySelectorAll('[data-product-thumb]').forEach((button) => {
      button.addEventListener('click', () => {
        const nextImage = button.dataset.productThumb;
